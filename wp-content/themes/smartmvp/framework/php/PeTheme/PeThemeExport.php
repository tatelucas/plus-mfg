...
class PeThemeExport {
	...
	public function rss2_head() {
		if (defined("PE_EXPORT_DEMO_CONTENT")) {
			...
			global $wpdb;
			$pt = $wpdb->posts;
			//$wp-db->query("UPDATE $pt SET $pt.post_status = 'inherit' WHERE $pt.post_type = 'attachment';");
			$wpdb->query($wpdb->prepare("UPDATE $pt SET $pt.post_status = '%s' WHERE $pt.post_type = '%s';",'inherit','attachment'));
		}
	}
	...
	public function export_wp() {
		...
		if (defined("PE_EXPORT_DEMO_CONTENT")) {
			global $wpdb;
			$pt = $wpdb->posts;
			//$wp-db->query("UPDATE $pt SET $pt.post_status = 'auto-draft' WHERE $pt.post_type = 'attachment';");
			$wpdb->query($wpdb->prepare("UPDATE $pt SET $pt.post_status = '%s' WHERE $pt.post_type = '%s';",'auto-draft','attachment'));
			//$wp-db->query("UPDATE $pt SET $pt.post_status = 'inherit' WHERE $pt.post_type = 'attachment' order by $pt.ID DESC LIMIT 4;");
			$wpdb->query($wpdb->prepare("UPDATE $pt SET $pt.post_status = '%s' WHERE $pt.post_type = '%s' order by $pt.ID DESC LIMIT 4;",'inherit','attachment'));
	...
	public function fixIDS() {
		...
		// fix shortcodes
		//$posts = $wp-db->get_results("SELECT ID,post_content FROM $pt WHERE $pt.post_content LIKE '%id=%'");
		$posts = $wpdb->get_results($wpdb->prepare("SELECT ID,post_content FROM $pt WHERE $pt.post_content LIKE '%s'",'%id=%'));
		if (is_array($posts)) {
		...
		if (!defined('PE_THEME_IMPORT_SKIP_META_FIX')) {
			$rep = array();

			foreach ($map as $key => $val) {
				if ($val != $key) {
					$key = sprintf('s:%d:"%s"',strlen("$key"),$key);
					$val = sprintf('s:%d:"%s"',strlen("$val"),$val);
					$rep[$key] = $val;
				}
			}
			...
			if (!empty($rep) && is_array($rep)) {
				...
				//$posts = $wp-db->get_results("SELECT post_id,meta_value FROM $pm WHERE $pm.meta_key = '$metaKey'");
				$posts = $wpdb->get_results($wpdb->prepare("SELECT post_id,meta_value FROM $pm WHERE $pm.meta_key = '%s'","$metaKey"));
				if (is_array($posts)) {
					foreach($posts as $post) {
						$value = $post->meta_value;
						$nvalue = strtr($value,$rep);
						if ($value != $nvalue) {
							update_post_meta($post->post_id,$metaKey,maybe_unserialize($nvalue));
						}
					}
				}
			}
