...
class PeThemeImporter extends WP_Import {
	...
	public function backfill_attachment_urls() {
		...
		//$attachment = wp_get_attachment_url($wp-db->get_var("SELECT ID FROM $pt WHERE $pt.post_type = 'attachment' LIMIT 1"));
		...
		//$posts = $wp-db->get_results("SELECT ID,post_content FROM $pt WHERE $pt.post_content LIKE '%$url%'");
	...
	public function remap_featured_images() {
		...
		// get attachments id
		//$aIDS = $wp-db->get_col("SELECT ID FROM $pt WHERE $pt.post_type = 'attachment'");
		$aIDS = $wpdb->get_col($wpdb->prepare("SELECT ID FROM $pt WHERE $pt.post_type = '%s'",'attachment'));

		// get post ids which have a feat image
		//$pIDS = $wp-db->get_col("SELECT post_id FROM $mt WHERE meta_key = '_thumbnail_id'");
		$pIDS = $wpdb->get_col($wpdb->prepare("SELECT post_id FROM $mt WHERE meta_key = '%s'",'_thumbnail_id'));
