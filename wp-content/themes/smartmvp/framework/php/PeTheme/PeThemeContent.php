...
class PeThemeContent {
	...
	public function splash($type) {
		$meta =& $this->meta();
		$splash = false;

		if (empty($meta->builder->builder["items"])) return false;

		foreach ($meta->builder->builder["items"] as $item) {
			if ($item["type"] === $type) {
				$splash = $item;
				break;
			}
		...
		$this->master->view->outputModule($splash);
	...
	public function pager($class = "col-md-12",$pages = false) {
